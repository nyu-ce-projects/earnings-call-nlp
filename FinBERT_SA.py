...
class FinBERT_SA:
    ...
    def __init__(self, pretrained_path=""):
        ...
        self.label = []
        self.preds = []
        self.device = -1 if torch.cuda.device_count()==0 else 0
    ...
    def train(self, train_dataset, val_dataset):
        ...
        self.trainer.train()
        self.pipeline = pipeline("text-classification", model=self.finbert, tokenizer=self.tokenizer, device=self.device)
        return self.trainer

    def checkSentiment(self, sentence, device):
        return self.pipeline(sentence,device = device)
